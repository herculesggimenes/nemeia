...
const channelCode = `// Planned agent/channels/world.ts — Eve 0.63.0 API sketch, not implemented.
import { defineChannel, POST } from "eve/channels";
import { worldBridge } from "../lib/world-bridge"; // Nemeia-specific boundary still to implement
...
// Do not treat this sketch as an exactly-once webhook or a ready-to-deploy adapter.`;

const sandboxCode = `// Planned agent/sandbox.ts — configuration sketch, not implementation.
import { defineSandbox } from "eve/sandbox";
import { justbash } from "eve/sandbox/just-bash";
import { nemeiaCommand, mountWorldProjection } from "./lib/world-bridge"; // trusted host-side integrations to implement

export default defineSandbox({
  backend: justbash({
    autoInstall: false, // pin compatible Eve + just-bash versions in the eventual worker
...
<div class="architecture-row"><strong>Nemeia world + systems</strong><div>Missions, agent/Unit authority, admission, evidence, execution receipts and independent robot-local control.</div></div>
  ...
  const definitions = [
    ["eve-channel", "World channel · Eve API sketch", "one continuation address per world/agent", "Planning only. Reviewed against Eve 0.63.0 bundled documentation; worldBridge helpers, delivery reconciliation and auth integration do not exist yet.", channelCode],
    ["eve-sandbox", "just-bash · sandbox sketch", "native backend plus narrow Nemeia extensions", "Planning only. The selected Eve version exposes customCommands and filesystem. The bridge, projection mount and egress restrictions still need implementation and security tests.", sandboxCode],
  ...
  const references = `<p class="reference-note">Reviewed against the Eve 0.63.0 npm package and its bundled docs: <a href="https://eve.dev/docs/channels/custom">custom channels</a> · <a href="https://eve.dev/docs/sandbox">sandboxes and just-bash</a> · <a href="https://eve.dev/docs/concepts/execution-model-and-durability">execution and durability</a> · <a href="https://eve.dev/docs/concepts/context-control">context control</a>. This is the selected plan, not an installed Nemeia runtime.</p>`;
  return section("eve-runtime",number,"Agent runtime · Eve","Eve runs the agent. A world channel and just-bash bridge expose Nemeia's domain model; no separate Nemeia thread runtime is needed. Planning only—no runtime or backend integration has been implemented.",`${ownership}<div class="abstraction-list">${principles.map((row,i)=>proseRow(i,...row)).join("\n")}</div><div class="object-list">${definitions}</div>${references}`);
}

...
test.describe("Nemeia smoke", () => {
  test("conversation page boots with shell, chat, and artifacts", async ({ page }) => {
    ...
    await expect(page.getByRole("treeitem", { name: /go2 microphone/i })).toHaveCount(0);
    ...
    await expect(page.getByText("Streams")).toHaveCount(0);
    await expect(page.getByText("Robot audio")).toBeVisible();
    await expect(page.getByRole("button", { name: "Choose file" })).toBeVisible();
    await expect(page.getByTestId("go2-control-pane").getByRole("button", { name: "Send" })).toBeVisible();
    await expect(page.getByRole("combobox", { name: "Audio input source" })).toHaveCount(0);
    await expect(page.getByRole("button", { name: "Start microphone" })).toHaveCount(0);
    await expect(page.getByRole("slider", { name: "Audio file position" })).toHaveCount(0);
    ...
    await expect(page.getByRole("button", { name: "Damp motors" })).toBeVisible();
    ...
    await expect(page.getByRole("button", { exact: true, name: "Speaker" })).toBeVisible();

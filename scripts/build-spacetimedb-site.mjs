...
const diagram = `<section class="diagram-section" id="how-it-works" aria-label="Nemeia model"><div class="diagram-inner section-inner"><h1 class="section-label">Nemeia / architecture</h1><div class="diagram-shell"><div class="diagram-header"><span class="diagram-label">WORLD AND DECISION LOOP</span><span>v0 · one agent · durable local world</span></div><div class="architecture-rows"><div class="architecture-row"><strong>World Master</strong><div>Describe missions + define objectives → assign the agent → grant Unit authority.</div></div><div class="architecture-row"><strong>Mission log</strong><div>One agent coordinates multiple assignments. Validated evidence advances each mission's objective progress.</div></div><div class="architecture-row"><strong>V0 success</strong><div>Accumulate and recover a progressive local map. Persistence supports every mission; additional Units and spatial reconciliation come later.</div></div></div><div class="diagram">${[
  ...
  ["Agent","One agent coordinates its mission log using accumulated local knowledge.","log + context → selected work"],
...
${section("example","07","End-to-end information flow","V0 planning walkthrough: World Master describes two missions and assigns Navigator. One agent coordinates its mission log; local observations become durable knowledge and validated objective progress. Persistence and recovery support both missions. These sketches perform no IO.",`<div class="example-list">${exampleRows.map(([id,title,summary,description],i)=>codeRow("example",i,title,summary,description,v0FlowCode[id],`flow-${id}`)).join("\n")}</div>`)}

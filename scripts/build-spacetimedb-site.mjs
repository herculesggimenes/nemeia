...
import { renderEve, flowCode } from "../docs/eve-content.mjs";
...
${section("tables","04","Database tables",`The world model uses ${tables.length} private tables, including shared missions, agents, Unit assignments, subscription policies and addressed messages. Eve owns runtime persistence; do not add parallel Nemeia thread, turn or step tables. Bridge delivery bookkeeping remains a separate implementation detail.`,`<div class="schema-list">${tableHtml}</div>`)}
...
${section("example","07","End-to-end information flow","A World Master assigns one mission to Navigator and Scene analyst. World changes wake Navigator through Eve; it inspects a just-bash projection and requests Go2 approach through the existing action boundary. Domain fixtures are type-checked; Eve handoffs are planning sketches. Nothing here performs IO.",`<div class="example-list">${exampleRows.map(([id,title,summary,description],i)=>codeRow("example",i,title,summary,description,flowCode[id] ?? region(sources.example,id),`flow-${id}`)).join("\n")}</div>`)}
